...
### 7. Review Notes (Exclude from the Final Plan)

#### Review Items

| Item / Question                       | Options                                                | Recommendation                                                                 | My Response |
| ------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------ | ----------- |
| eBay Browse API production access     | Sandbox only, production approved, or blocked          | Verify early before depending on live eBay results for daily use               |             |
| Initial `ProductSearchRequest` fields | Minimal fields or broader eBay-backed controls         | Start with query, price range, condition, buying option, category ID, limit    |             |
| Claude Design timing                  | Before initial UI, after utilitarian UI, or much later | Build utilitarian UI first; integrate supplied design system when available    |             |
| Notifications in MVP 2                | Include, defer, or remove                              | Defer until scheduled collection proves useful                                 |             |
| Craigslist source path                | RSS/feed, API/third-party, manual import, or defer     | Validate later before implementation                                           |             |
| Facebook future strategy              | Never, external repo module, or same-repo later        | Keep deferred and external-module-friendly; do not implement in MVP 1 or MVP 2 |             |

#### Other Assumptions

1. `Product` is the normalized app object and replaces `Listing` as the central domain term.
2. The first source is eBay through the Buy Browse API.
3. The UI does not call eBay directly; eBay access stays behind the Next.js server boundary.
4. The app stores normalized Products only and does not store raw source response JSON.
5. Older Products missing future fields are acceptable because the app is focused on new Product discovery.
6. Docker is not required for MVP 1 unless implementation reveals a clear need.
7. The dashboard starts utilitarian and can be visually refined after a Claude Design system/design is supplied.
8. Review states are `new`, `interested`, and `rejected`.
9. Facebook remains out of implementation scope until explicitly reintroduced.
10. The source adapter boundary should stay small and practical, not a full plugin framework.
11. SQLite with Drizzle ORM and drizzle-kit is the default MVP 1 persistence stack.
    **My Comments:** [I fill in any reactions here, referencing assumptions by number.]

#### Additional Requirements / New Concepts

[Free-form space — add any new thoughts, requirements, or ideas here.]
